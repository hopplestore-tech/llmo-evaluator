export default async function handler(req, res) {
  ...
  try {
    ...
    if (pageText.length < 80) pageText = `URL: ${url} (ページ取得制限あり)`;
    ...
    const evalRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 1000,
        system: `あなたはLLMO・AIO専門家です。WebページをAI参照されやすさの観点で評価し、JSONのみ返してください。前後の説明不要。
形式: {"total_score":整数,"verdict":"15字以内","summary":"60字以内","axes":{"structure":{"score":整数,"comment":"40字以内"},"credibility":{"score":整数,"comment":"40字以内"},"depth":{"score":整数,"comment":"40字以内"},"citability":{"score":整数,"comment":"40字以内"},"eeat":{"score":整数,"comment":"40字以内"}},"improvements":["提案1","提案2","提案3"]}`,
        messages: [{ role: 'user', content: `以下を評価してください:\n\n${pageText.slice(0, 3000)}` }]
      })
    });
    const evalData = await evalRes.json();
    const raw = (evalData.content || []).filter(b => b.type === 'text').map(b => b.text).join('');
    const s = raw.indexOf('{'), e = raw.lastIndexOf('}');
    const result = JSON.parse(s !== -1 ? raw.slice(s, e + 1) : raw);
